...
function App() {
  ...
  const tipAmount = bill > 0 ? ((bill * tipPercentage) / 100 / people).toFixed(2) : "0.00";
  const totalAmount = bill > 0 ? (bill / people + parseFloat(tipAmount)).toFixed(2) : "0.00";

  // Reset function to clear inputs
  const handleReset = () => {
    setBill(0);
    setTipPercentage(15); // Reset to default tip percentage
    setPeople(1);
  };
    ...
    <div className="container">
      ...
      <div className="calculator">
        ...
        <div className="results">
          <Result tipAmount={tipAmount} totalAmount={totalAmount} handleReset={handleReset}/>
        </div>
      </div>
    </div>
  );
